...
public class WorkLoad extends WorkLoadDescription implements ReliabilityParameters {
	...
	private static final String FLOW_WARNING = "\n\tWarning! Bad situation: "
			+ "Flow %s doesn't exist but trying to ";
	...
	private String name ; 
	private ArrayList<String> flowNamesInOriginalOrder = new ArrayList<>(); // array to hold names of flows to preserve their order
	...
	WorkLoad (Double m, Double e2e, String inputFileName) {
		...
		/*
		 * Read input file, build the AST of graph
		 * and the listener will build the node and flow data objects
		 */
	...
	private void setDefaultParameters() {
		...
		flowNamesInOriginalOrder = new ArrayList<>(); // array to hold names of flows to preserve their order
	...
	public Integer getMaxPhase() {
		var queue = new
		SchedulableObjectQueue<Flow>(new MaxPhaseComparator<Flow>(),flows.values());
	...
	public Integer getMinPeriod() {
		var queue = new
		SchedulableObjectQueue<Flow>(new PeriodComparator<Flow>(),flows.values());
	...
	public void addFlow (String flowName ) {
		/*
		 * add a new flow node to the Flows dictionary.
		 * Only name, priority, and index are changed from default values
		 * priority is set to the number of flows already added (index), 0 for first flow
		 * This will create a default priority equal to the order listed in the
		 * input graph file. We also set index to the same value so we can preserve that
		 * order as a secondary sort key.
		 * The initalPriority field is probably not needed, but it might be useful
		 * in the future?? If the optional flow parameters (priority, period, ...)
		 * is set, then this default priority will be over written
		 */
		...
		if (!Utilities.isInteger(flowName)  && intForFlowNames){ 
			intForFlowNames = false;  // set false because name not is a number; && above makes sure we only set it once
		}
		flowNamesInOriginalOrder.add(flowName);
	}

	public Boolean isIntForNodeNames ()  { // returns false if any node name is not an int
		return intForNodeNames;
	}

	public Boolean isIntForFlowNames ()  { // returns false if any flow name is not an int
		return intForFlowNames;
	}
	...
	public void addNodeToFlow(String flowName, String nodeName) {
		...
		if (!nodes.containsKey(nodeName)) { // create the node and add it to nodes if map doesn't have this node already. 
		...
		/*
		 * Node is now created and in the nodes map
		 * Next we need to get the current flow and add this node to that flow by 
		 * appending it to the node array for that flow 
		 */
	...
	public void setFlowsInPriorityOrder() {
		...
		List<Flow> sortedByIndex = 
				unsortedFlows.stream().sorted(Comparator.comparing(Flow::getIndex)).collect(Collectors.toList());
		// Now sort by primary key, which is priority in this case
		List<Flow> sortedFlows = 
				sortedByIndex.stream().sorted(Comparator.comparing(Flow::getPriority)).collect(Collectors.toList());
		// Finally, create a new flowNamesInPriorityOrder that contains the flow names in the requested order
		flowNamesInPriorityOrder = new ArrayList<>();
	...
	public void setFlowsInDMorder() {
		...
		List<Flow> sortedByPriority = 
				unsortedFlows.stream().sorted(Comparator.comparing(Flow::getPriority)).collect(Collectors.toList());
		/* Now sort by primary key, which is deadline in this case */
		List<Flow> sortedFlows = 
				sortedByPriority.stream().sorted(Comparator.comparing(Flow::getDeadline)).collect(Collectors.toList());
		/*
		 * Finally, create a new flowNamesInPriorityOrder that contains the 
		 * flow names in the requested order
		 */
		flowNamesInPriorityOrder = new ArrayList<>();
		sortedFlows.forEach((node)->flowNamesInPriorityOrder.add(node.getName())); 
	}
	...
	public void setFlowsInRMorder() {
		...
		List<Flow> unsortedFlows = flows.values().stream().collect(Collectors.toList());
		// Now sort by a secondary key, which is priority in this case
		List<Flow> sortedByPriority = unsortedFlows.stream().sorted(Comparator.comparing(Flow::getPriority)).collect(Collectors.toList());
		// Now sort by primary key, which is period in this case
		List<Flow> sortedFlows = sortedByPriority.stream().sorted(Comparator.comparing(Flow::getPeriod)).collect(Collectors.toList());
		// Finally, create a new flowNamesInPriorityOrder that contains the flow names in the requested order
		flowNamesInPriorityOrder = new ArrayList<>();
		sortedFlows.forEach((node)->flowNamesInPriorityOrder.add(node.getName())); 
	}
	...
	private void finalizeFlowWithE2eParameters(String flowName) {
		...
		if (flowNode != null) {
			...
			if (nHops < 1) {
				/* number of hops in flow, but make sure it will be at least 1, 
				 * else it isn't a flow! || was -1 at end
				 */
				nHops = 2;  
			}
			Double nTx = 1.0; // set nTx to 1 by default (1 transmission per link required at a minimum and when m == 1.0
			if (m < 1.0) {
				/* 
...
private void finalizeFlowWithFixedFaultTolerance(String flowName) {
	...
	private ArrayList<Integer> getFixedTxPerLinkAndTotalTxCost(Flow flow) {
		 ...
		 * Because we don't know which nodes will send the message
		 * over an edge, we give the cost to each node. 
		 */
		for (int i = 0; i < nNodesInFlow; i++) {
			txArrayList.add(numFaults+1);
		}
		/* now compute the maximum # of TX, assuming at most 
		 * numFaults occur on an edge per period, and
	...
	private ArrayList<Integer> nTxPerLinkAndTotalTxCost(Flow flow, Double e2e, Double M, boolean optimizationRequested)  {
		var nodesInFlow = flow.nodes;
		var nNodesInFlow = nodesInFlow.size(); // The last entry will contain the worst-case cost of transmitting E2E in isolation
		// var nPushes = Array(repeating: 0, count: nNodesInFlow+1);
		var nPushes = new Integer[nNodesInFlow+1];  // Array to track nPushes for each node in this flow (same as nTx per link)
		Arrays.fill(nPushes,0); // initialize to all 0 values
		var nHops = nNodesInFlow-1;
		// minLinkReliablityNeded is the minimum reliability needed per link in a flow to hit E2E reliability for the flow
		Double minLinkReliablityNeded =  Math.max(e2e, Math.pow(e2e,(1.0/(double)nHops))); // use max to handle rounding error when e2e == 1.0
		...
		Double e2eReliabilityState = currentReliabilityRow[nNodesInFlow-1]; // the analysis will end when the 2e2 reliability metrix is met, initially the state is not met and will be 0 with this statement
		var timeSlot = 0 ; // start time at 0
		while (e2eReliabilityState < e2e) { // change to while and increment increment timeSlot becuase we don't know how long this schedule window will last
			var prevReliabilityRow = currentReliabilityRow;
			currentReliabilityRow = newReliabilityRow.toArray(new Double[newReliabilityRow.size()]); // would be reliabilityWindow[timeSlot] if working through a schedule
			...
			for (int nodeIndex = 0; nodeIndex < (nNodesInFlow-1); nodeIndex++) { // loop through each node in the flow and update the sates for each link (i.e., sink->src pair)
				...
				Double nextSnkState;
				if ((prevSnkNodeState < minLinkReliablityNeded)  && prevSrcNodeState > 0) { // do a push until PrevSnk state > e2e to ensure next node reaches target E2E BUT skip if no chance of success (i.e., source doesn't have packet)
					nextSnkState = ((1.0-M) * prevSnkNodeState) + (M * prevSrcNodeState); // need to continue attempting to Tx, so update current state
					nPushes[nodeIndex] += 1; // increment the number of pushes for for this node to snk node
				} else {
					nextSnkState = prevSnkNodeState; // snkNode has met its reliability. Thus move on to the next node and record the reliability met
				}

				if (currentReliabilityRow[flowSrcNodeindex] < prevReliabilityRow[flowSrcNodeindex]) {  // probabilities are non-decreasing so update if we were higher by carring old value forward
					currentReliabilityRow[flowSrcNodeindex] = prevReliabilityRow[flowSrcNodeindex]; // carry forward the previous state for the src node, which may get over written later by another instruction in this slot
				}
		...
		nPushes[nNodesInFlow] = size; // The total (worst-case) cost to transmit E2E in isolation with specified reliability target is the number of rows in the reliabilityWindow
	...
	public String[] getNodesOrderedAlphabetically () {
		...
		for (int i = 0; i < nodeNames.length; i++) {
			...
			if (Utilities.isInteger(nodeName)  && allIntNames){ 
				...
				allIntNames = false;  // set false because name not is a number; && above makes sure we only set it once
	...
	public Integer getHyperPeriod()  {
		...
		for (String flowName : getFlowNames()) {
			...
			hyperPeriod = Utilities.lcm(hyperPeriod, getFlowPeriod(flowName)); // Find LCM of hyperPeriod so far and the current period, then update the hyperPeriod with that value
		}
		return hyperPeriod;
	}
