// Course CS2820 Authentication Tag: 3JDpgcyvRSHACa+sL04B/z/NQQ0fzmCipAX4CiZ0BzA4z+xBWxMSay+oQB2rtaBtNJIXAi/LjI1Uln8jSEND5UfX6PWI41RoTCgMV8EEzmo=
...
nChannels:	16
Time Slot	A	B	C
0	if has(F0) push(F0: A -> B, #4)	wait(#4)	sleep
1	wait(#8)	if has(F0) push(F0: B -> C, #8) else pull(F0: A -> B, #8)	wait(#8)
2	wait(#9)	if has(F0) push(F0: B -> C, #9) else pull(F0: A -> B, #9)	wait(#9)
3	sleep	if has(F0) push(F0: B -> C, #10)	wait(#10)
4	sleep	wait(#4)	if has(F1) push(F1: C -> B, #4)
5	wait(#11)	if has(F1) push(F1: B -> A, #11) else pull(F1: C -> B, #11)	wait(#11)
6	wait(#12)	if has(F1) push(F1: B -> A, #12) else pull(F1: C -> B, #12)	wait(#12)
7	wait(#13)	if has(F1) push(F1: B -> A, #13)	sleep
8	sleep	sleep	sleep
9	sleep	sleep	sleep
